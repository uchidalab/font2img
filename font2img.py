...
class font2img():
    ...
    def __init__(self, src_font_dir_path, src_chars_txt_path, dst_dir_path, canvas_size, font_size, is_center, is_maximum, is_binary, ext, is_verbose):
        ...
        if is_maximum:
            self.draw_char_func = self._draw_char_maximum
        elif is_center:
            self.draw_char_func = self._draw_char_center
        else:
            self.draw_char_func = self._draw_char
    ...
    def run(self):
        for font_path in self.font_paths:
            ...
            dst_img_dir_path = os.path.join(self.dst_dir_path, os.path.basename(os.path.splitext(font_path)[0]))
            failure_chars = list()
            ...
            for i, c in enumerate(self.chars):
                if self.is_verbose:
                    progbar.update(i + 1)
                img = self.draw_char_func(c, font_path, self.canvas_size, self.font_size)
                if self._is_white(img):
                    failure_chars.append(c)
                else:
                    if c in AVOIDED_CHARS:
                        c = ord(c)
                    img.save(os.path.join(dst_img_dir_path, '{}.{}'.format(c, self.ext)))
            if failure_chars:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--verbose', dest='is_verbose', action='store_true', help='Progress')
